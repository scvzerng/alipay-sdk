 ...
 */
@ApiModel(description = " 口碑广告系统主键结果")
public class KbAdvertIdentifyResponse extends AlipayObject {

    private static final long serialVersionUID = 2552231619919565296L;

    /**
     * 根据benefit_type，确定ID含义
     * SINGLE_VOUCHER时，benefit_ids为券ID
     */
    @ApiModelProperty(notes = " 根据benefit_type，确定ID含义SINGLE_VOUCHER时，benefit_ids为券ID")
     ...
     * SINGLE_VOUCHER：单券
     */
    @ApiModelProperty(notes = " 发放权益类型SINGLE_VOUCHER：单券")
     ...
     */
    @ApiModelProperty(notes = " 返回码success: 成功invalid-arguments: 无效参数retry-exception: 异常请重试isv.user-already-get-voucher：用户已经领过该券，同时券状态为有效isv.item_inventory_not_enough:优惠领光了isv.item_not_in_this_shop_sales:不是该商家的优惠，不能领取isv.voucher_activity_not_started:活动未开始isv.voucher_activity_expired:活动已结束isv.crowd_limit_not_match_error:暂无领取资格，详情请咨询商家isv.member_crowd_limit_not_match_error:会员专属，请先注册会员")
    @ApiField("code")
    private String code;
     ...
     * JSON格式数据，需要ISV自行解析
     */
    @ApiModelProperty(notes = " JSON格式数据，需要ISV自行解析")
    @ApiField("ext_info")
